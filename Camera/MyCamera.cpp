...
{
	forward = vector3(-sin(angleY) * cos(angleX), -sin(angleX), cos(angleX) * -cos(angleY));
	m_v3Position += glm::normalize(forward) * a_fDistance;
	m_v3Target += glm::normalize(forward) * a_fDistance;
	m_v3Above += glm::normalize(forward) * a_fDistance;
}

void MyCamera::MoveVertical(float a_fDistance)
{	 
	forward = vector3(-sin(angleY) * cos(angleX), -sin(angleX), cos(angleX) * -cos(angleY));
	right = vector3(-cos(angleY), 0.0f, sin(angleY));
	m_v3Position += glm::normalize(glm::cross(forward, right)) * a_fDistance;
	m_v3Target += glm::normalize(glm::cross(forward, right)) * a_fDistance;
	m_v3Above += glm::normalize(glm::cross(forward, right)) * a_fDistance;
...
void MyCamera::Rotation() {
	forward = vector3(-sin(angleY) * cos(angleX), -sin(angleX), cos(angleX) * -cos(angleY));
	right = vector3(-cos(angleY), 0.0f, sin(angleY));

	m_v3Target =  forward + m_v3Position;
}
